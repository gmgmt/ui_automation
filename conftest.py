...
def start_stop_app(driver):
    ...
    # check_pkg = AdbTools().check_pkg(package_name=package_name)
    # if pkg_list and check_pkg:
    #     pkg_name = pkg_list[0]
    #     pkg_path = os.path.join(base_path, pkg_name)
    #     logging.info("install app: %s", str(package_name))
    #     AdbTools().install(pkg_path)

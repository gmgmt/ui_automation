...
class BasePage(object):
    ...
    def __init__(self, driver: Union[Device, wda.Client], platform=""):
        self.driver = driver
        app_name = platform if platform else self.platform
        # self.platform = app_name
        self.platform = ReadConfig().get_platform(app_name)
        self.implicitly_wait = ReadConfig().get_implicitly_wait(app_name)
        self.check_error_toast = ReadConfig().get_check_error_toast(app_name)

    def find_element(self, retry_times=_MAX_RETRY_TIMES, **locator):
        """
        定位元素，支持以下参数：
            ...
            if self.platform == "android":
                if "xpath" in locator.keys():
                    ...
                    if "child" in locator.keys():
                        ...
                        elements = self.driver(**self.handle_resourceId(**locator))
                    if len(elements) == 0:
                        elements.must_wait()
                    elif len(elements) > 1:
                        logging.warning("More than one element was found, please reselect the element positioning "
    ...
    def check_element_existence(self, **locator):
        """ 检查元素存在性"""
        if self.driver(**locator).exists(timeout=5.0):
            logging.info("check element exist: %s %s", str(locator), "True")
            return True
        else:
